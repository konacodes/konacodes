...
export function renderAdmin(posts: Post[]): string {
  const postsList = posts.length > 0
    ...
    : `
        <div class="empty-state">
          <p>no reviews yet</p>
          <a href="/admin/new" class="btn btn-primary">write your first review</a>
        </div>
      `;
  ...
  return `
    ...
    <head>
      ${baseHead}
      <title>admin — kona_/films</title>
      <style>${adminStyles}</style>
    </head>
    <body>
      <div class="container">
        <header class="admin-header animate-in">
          <h1><span>kona_</span>/films admin</h1>
          <a href="/admin/new" class="btn btn-primary">new review</a>
        </header>
...
function renderEditorPage(post: Post, isNew: boolean): string {
  return `
    ...
    <head>
      ${baseHead}
      <title>${isNew ? 'new review' : `edit: ${post.title}`} — kona_/films</title>
      <style>${adminStyles}</style>
    </head>
    <body>
      <div class="container">
        <div class="editor-container">
          <header class="editor-header animate-in">
            <h1>${isNew ? 'new review' : 'edit review'}</h1>
          ...
          <form id="post-form" class="animate-in delay-1">
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="text" id="title" name="title" value="${escapeHtml(post.title)}" placeholder="The Film Name (Year)" required>
              </div>
              <div class="form-group">
            ...
            <div class="form-group">
              <label for="excerpt">one-liner <span style="color: var(--color-text-muted); font-weight: 400;">(optional)</span></label>
              <input type="text" id="excerpt" name="excerpt" value="${escapeHtml(post.excerpt || '')}" placeholder="quick take on the film">
            </div>
            ...
            <div class="editor-main">
              <div class="editor-pane">
                <h3>markdown</h3>
                <textarea id="content" name="content" placeholder="write your review in markdown...">${escapeHtml(post.content)}</textarea>
              </div>
              <div class="editor-pane">
                <h3>preview</h3>
